...
abstract public class RefactoringType {
    ...
    public List<AbstractCandidateRefactoringGroup> getRefactoringOpportunities(ProjectInfo projectInfo, ProgressIndicator indicator) {
        ...
        List<ExtractClassCandidateGroup> extractClassCandidateGroups = new ArrayList<>();
        for(Object candidate : notAbstractRefactoringOpportunities) {
            ExtractClassCandidateGroup candidate1 = (ExtractClassCandidateGroup) candidate;
            Collections.sort(candidate1.getCandidates());
            extractClassCandidateGroups.add(candidate1);
        }
        Collections.sort(extractClassCandidateGroups);
        List<AbstractCandidateRefactoringGroup> result = new ArrayList<>();
        for (ExtractClassCandidateGroup candidate : extractClassCandidateGroups) {
