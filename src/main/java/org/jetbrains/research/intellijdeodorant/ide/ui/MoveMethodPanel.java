...
class MoveMethodPanel extends JPanel {
    ...
    private void setupTableLayout() {
        final TableColumn selectionColumn = table.getTableHeader().getColumnModel().getColumn(MoveMethodTableModel.SELECTION_COLUMN_INDEX);
        selectionColumn.setMaxWidth(30);
        selectionColumn.setMinWidth(30);

        final TableColumn dependencies = table.getTableHeader().getColumnModel().getColumn(MoveMethodTableModel.SELECTION_COLUMN_INDEX);
        ...
        final Task.Backgroundable backgroundable = new Task.Backgroundable(project, IntelliJDeodorantBundle.message("feature.envy.detect.indicator.status"), true) {
            ...
            public void run(@NotNull ProgressIndicator indicator) {
                ApplicationManager.getApplication().runReadAction(() -> {
                    ...
                    model.setCandidateRefactorings(candidates);
    ...
    private void onDoubleClick(InputEvent e) {
        ...
        if (selectedRow != -1 && (selectedColumn == MoveMethodTableModel.ENTITY_COLUMN_INDEX || selectedColumn == MoveMethodTableModel.MOVE_TO_COLUMN_INDEX)) {
