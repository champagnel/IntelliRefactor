...
public class DistanceMatrix {
    ...
    public double[][] getJaccardDistanceMatrixForFeatureEnvy(List<Entity> entities, MyClass myClass) {
        double[][] jaccardDistanceMatrix = new double[entities.size()][entities.size()];
        for (int i = 0; i < jaccardDistanceMatrix.length; i++) {
            for (int j = 0; j < jaccardDistanceMatrix.length; j++) {
                if (i != j) {
    ...
    public double getJaccardDistanceMatrixFromSourceToOthers(MyClass sourceClass){
        ArrayList<Entity> entitiesOfSource = new ArrayList<>();
        entitiesOfSource.addAll(sourceClass.getAttributeList());
        entitiesOfSource.addAll(sourceClass.getMethodList());
        Set<String> sourceSet = new HashSet<>();
        for (Entity e : entitiesOfSource) {
            sourceSet.addAll(e.getFullEntitySet());
        }

        ArrayList<Entity> entitiesOfOthers = new ArrayList<>();
        for(MyClass myClass : classList) {
            if(!sourceClass.getName().equals(myClass.getName())) {
                entitiesOfOthers.addAll(myClass.getAttributeList());
                entitiesOfOthers.addAll(myClass.getMethodList());
            }
        }
        double result = 0;
        /*for(Entity entityOfSource : entitiesOfSource) {
            for(Entity entityOfOthers : entitiesOfOthers){
                result += DistanceCalculator.getDistance(entityOfSource.getFullEntitySet(), entityOfOthers.getFullEntitySet());
            }
        }**/
        for(Entity entityOfOthers : entitiesOfOthers){
            result += DistanceCalculator.getDistance(entityOfOthers.getFullEntitySet(), sourceSet);
        }
        if(result == 0 || entitiesOfOthers.size() == 0) {
            return 1.0D;
        }
        result = result / (double) entitiesOfOthers.size();
        return result;
    }

    public double getJaccardDistanceMatrixFromSourceToOthersForFeatureEnvy(MyClass sourceClass, MyClass sourceClass1, MyMethod sourceMethod, List<Entity> entities){
        double result = 0;
        ArrayList<Entity> entitiesOfOthers = new ArrayList<>();
        if(sourceClass.getName().equals(sourceClass1.getName())){
            entitiesOfOthers.add(sourceMethod);
            for(MyClass myClass : classList) {
                if (!sourceClass.getName().equals(myClass.getName())) {
                    entitiesOfOthers.addAll(myClass.getAttributeList());
                    entitiesOfOthers.addAll(myClass.getMethodList());
            ...
            for(Entity entityOfSource : entities) {
                for(Entity entityOfOthers : entitiesOfOthers){
                    result += DistanceCalculator.getDistance(entityOfSource.getFullEntitySet(), entityOfOthers.getFullEntitySet());
                }
            }
            ...
            for(MyClass myClass : classList) {
                if(sourceClass1.getName().equals(myClass.getName())){
                    entitiesOfOthers.addAll(myClass.getAttributeList());
                    entitiesOfOthers.addAll(myClass.getMethodList());
                    entitiesOfOthers.remove(sourceMethod);
                } else if(!sourceClass.getName().equals(myClass.getName())) {
                    entitiesOfOthers.addAll(myClass.getAttributeList());
                    entitiesOfOthers.addAll(myClass.getMethodList());
                }
            }
