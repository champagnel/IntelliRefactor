...
public class MoveMethodCandidateRefactoring extends CandidateRefactoring implements Comparable<MoveMethodCandidateRefactoring> {
    ...
    private final List<MethodObject> additionalMethods;
    private String movedMethodName;
    private final FeatureEnvyVisualizationData visualizationData;
    private double entityPlacementOfSystem;
    private double originalEP;
        ...
        for (MethodInvocationObject methodInvocation : methodInvocations) {
            if (methodInvocation.getOriginClassName().equals(sourceClass.getClassObject().getName()) &&
                ...
                if (invokedMethod != null && !systemMemberAccessed && !additionalMethodsToBeMoved.containsKey(methodInvocation.getMethodInvocation())){
                    ...
                    additionalMethods.add(invokedMethod);
                }
            }
        }
    ...
    public int compareTo(MoveMethodCandidateRefactoring other) {
        if(this.entityPlacementOfSystem < other.entityPlacementOfSystem)
            return -1;
        else if(this.entityPlacementOfSystem == other.entityPlacementOfSystem)
            return 0;
        return 1;
    }
    ...
    public double getEntityPlacementOfSystem() {
        return entityPlacementOfSystem;
    }

    public void setEntityPlacementOfSystem(double entityPlacementOfSystem) {
        this.entityPlacementOfSystem = entityPlacementOfSystem;
        //DirectedGraph directedGraph = new DirectedGraph(system);
        //double entityPlacementOfSystem = directedGraph.caculateFeatureEnvyEntityPlacementOfSystem(sourceClass, targetClass, sourceMethod, additionalMethods);
        //this.entityPlacementOfSystem = entityPlacementOfSystem;
    }

    public double getOriginalEP() {
        return originalEP;
    }

    public void setOriginalEP(double originalEP) {
